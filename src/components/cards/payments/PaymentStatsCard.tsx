  ...
  const stats = React.useMemo(() => {
    ...
    const totalNegative = payments
      ...
      .reduce((sum, p) => {
        if (p.status === "partial") {
          const paidAmount =
            p.partialPayments?.reduce((sum, pp) => sum + pp.amount, 0) || 0;
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <span className="font-medium text-gray-900">
              {formatCurrency(stats.sportTotal)}
            </span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600">Cuotas Societarias</span>
            <span className="font-medium text-gray-900">
              {formatCurrency(stats.societaryTotal)}
            </span>
          </div>
          <div className="border-t pt-2">
            <div className="flex justify-between items-center font-medium">
              ...
              <span className="text-[#FFD700]">
                {formatCurrency(stats.sportTotal + stats.societaryTotal)}
              </span>
            </div>
          </div>
        </div>
