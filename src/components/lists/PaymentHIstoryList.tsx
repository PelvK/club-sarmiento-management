...
export const GenerationHistoryList: React.FC<GenerationHistoryListProps> = ({
  ...
  const [selectedGeneration, setSelectedGeneration] =
    useState<PaymentGeneration | null>(null);
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {currentGenerations.length > 0 ? (
                currentGenerations.map((generation) => {
                  ...
                  const totalMembers = generation.stats.totalMembersCount ?? 0;
                  const coveredMembers =
                    generation.stats.coveredMembersCount ?? 0;
                  const memberPct =
                    totalMembers > 0
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="text-xs font-medium text-gray-600">
                          {generation.stats.principalSportsCount} principales •{" "}
                          {generation.stats.secondarySportsCount} secundarias
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-bold text-gray-900">
                      ...
                      <td className="px-6 py-4">
                        <div className="space-y-3 min-w-[200px]">
                          {/* Barra cuotas pagadas */}
                          <div className="space-y-1">
                            <div className="flex items-center justify-between gap-2">
                              <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide flex items-center gap-1">
                                <LucideFileSpreadsheet className="w-3 h-3" />
                                Cuotas
                              </span>
                              <span
                                className={`text-xs font-bold ${getProgressTextColor(progress.percentage)}`}
                              >
                          ...
                          {totalMembers > 0 && (
                            <div className="space-y-1">
                              <div className="flex items-center justify-between gap-2">
                                ...
                                <span
                                  className={`text-xs font-bold ${getProgressTextColor(memberPct)}`}
                                >
                                  {coveredMembers}/{totalMembers} ·{" "}
                                  {memberPct.toFixed(0)}%
                                </span>
                              </div>
                              <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                                <div
                                  className={`h-full rounded-full transition-all duration-500 ${getProgressColor(memberPct)}`}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`px-3 py-1.5 text-xs font-bold rounded-lg ${getStatusBadgeClass(generation.status)}`}
                          >
